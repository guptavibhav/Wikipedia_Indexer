...
public class Stopwords implements TokenizerRule {
	...
	private Set<String> stopwordSet = new HashSet<String>(
			Arrays.asList(new String[] { "a", "about", "above", "above",
					...
					"at", "back", "be", "became", "because", "become",
	...
	public void apply(TokenStream stream) throws TokenizerException {
	    ...
	    for (String stpword : stopwordSet)
	    {
	    	Regex = Regex + "|"+stpword;
	    }
	   ...
	   /* Pattern p = Pattern.compile("[\\s]"+Regex+"(?!\\s)");
	    ...
	    String tempstr=null;
	    while (!str.equals(tempstr))
	    {
	    tempstr = str;
	    str = str.replaceAll("[\\s]"+Regex+"(?![\\s])", "");
	    str = str.replaceAll("(?<![\\s])"+Regex+"[\\s]", "");
	    str = str.replaceAll("(?<=[\\s])"+Regex+"[\\s]", "");
	    str = str.replaceAll("(?<![\\s])"+Regex+"(?![\\s])", "");
	    }
